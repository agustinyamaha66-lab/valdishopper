...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  const toggle =
      typeof setSidebarOpen === "function" ? () => setSidebarOpen((v) => !v) : () => {};
  ...
  const NavGroup = ({ title, icon: Icon, children, activePaths = [] }) => {
    ...
    return (
        <div className="mb-2">
          ...
          `}
          >
            <div className="pl-3 border-l border-white/10 ml-6 space-y-1">{children}</div>
          </div>
        </div>
    );
  };
          ...
          <nav className="p-3 overflow-y-auto h-[calc(100vh-64px)] scrollbar-hide">
            ...
            <NavGroup
                ...
                activePaths={[
                  ...
                  "/devoluciones",
                  "/devoluciones-excel", // ✅ NUEVO
                  "/catastro-patentes",
                ]}
            >
              <NavItem to="/transporte" icon={Map} label="Torre de Control" />
              <NavItem to="/transporte-dashboard" icon={BarChart2} label="Dashboard Transporte" />
              <NavItem to="/devoluciones" icon={RotateCcw} label="Devoluciones" />

              {/* ✅ NUEVO: Importación/visor Excel */}
              <NavItem to="/devoluciones-excel" icon={FileSpreadsheet} label="Devoluciones Excel" />

              {esAdmin && <NavItem to="/catastro-patentes" icon={Car} label="Catastro Patentes" />}
            </NavGroup>

            {/* GRUPO HD (Solo si tiene permisos) */}
            {esRuteo && (
                <NavGroup title="Gestión HD" icon={Layers} activePaths={["/ruteo"]}>
            ...
            >
              ...
              {esAdmin && <NavItem to="/bitacora-dashboard" icon={BarChart2} label="Dashboard Incidencias" />}
            </NavGroup>
            ...
            {/* GRUPO FINANZAS */}
            {esFinanzas && (
                <NavGroup title="Finanzas" icon={TrendingUp} activePaths={["/finanzas", "/reportes-financieros"]}>
            ...
            {/* GRUPO ADMIN */}
            {esAdmin && (
                <NavGroup title="Configuración" icon={Settings} activePaths={["/usuarios"]}>
            ...
            <div className="mt-8 px-2 mb-4">
              <div className="p-4 rounded-xl bg-white/5 border border-white/5 text-center backdrop-blur-sm">
                ...
                <p className="text-[10px] text-blue-200/40 font-mono tracking-wider">Agustin v2</p>
              </div>
            </div>
          </nav>
        </aside>
