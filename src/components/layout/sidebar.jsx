...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  // Si no te pasan estado desde AppLayout, no revienta (fallback)
  ...
  const isActive = (path) =>
    location.pathname === path
      ? "bg-white/10 border-l-4 border-[#d63384] text-white"
      : "text-gray-400 hover:text-white hover:bg-white/5";

  // Para mobile: cerrar sidebar al elegir link
  const closeOnMobile = () => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-64 bg-[#0f172a] shadow-2xl transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <nav className="p-4 space-y-2 overflow-y-auto h-[calc(100vh-64px)]">
          <Link
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-r-lg transition-all ${isActive("/")}`}
          >
            <HomeIcon size={20} /> <span className="font-bold text-sm">Inicio</span>
          </Link>
          ...
          <Link
            to="/ruteo"
            onClick={closeOnMobile}
            className={`flex items-center gap-3 px-4 py-3 rounded-r-lg transition-all ${isActive("/ruteo")}`}
          >
          ...
          {esFinanzas && (
            <>
              ...
              {/* OJO: aquí alinéalo con tu ruta real del App.jsx */}
              <Link
          ...
          {esAdmin && (
            <>
              ...
              >
                <BarChart2 size={20} />{" "}
                <span className="font-bold text-sm">Registro Incidencias</span>
              </Link>
