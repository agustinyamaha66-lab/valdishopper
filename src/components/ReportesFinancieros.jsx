...
const PageHeader = ({ title, subtitle, icon: Icon, children }) => (
  <div className="relative overflow-hidden rounded-2xl border border-white/20 shadow-xl mb-6">
    ...
    <div className="relative z-10 px-6 py-5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-white/60 mb-1">{subtitle}</p>
        <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight flex items-center gap-3">
          {Icon && <div className="p-2 bg-white/10 rounded-lg backdrop-blur-sm border border-white/10"><Icon size={24} className="text-[#d63384]" /></div>}
          {title}
        </h1>
      </div>

      {/* Área de acciones (Children) */}
      {children && (
        <div className="flex flex-wrap gap-2 items-center bg-white/5 p-1.5 rounded-xl border border-white/10 backdrop-blur-sm">
          {children}
        </div>
      )}
    </div>
...
export default function ReportesFinancieros() {
  ...
  // Estado para el modo de vista (SEMANA, MES, HISTORICO)
  ...
  const registrosFiltrados = useMemo(() => {
    ...
    return registros.filter((r) => {
      // ✅ FILTRO EXTRA: Excluir explícitamente HD de la vista y gráficos
      if (r.servicio === "HD") return false;
  ...
  const descargarExcelPorRango = async () => {
    ...
    // NOTA: Para el excel también filtramos HD en el cliente si es necesario,
    // pero aquí lo dejamos traer todo lo que pida el filtro de servicio.
    // Si seleccionas "TODOS", traerá HD si existen. Si quieres que JAMÁS se descargue HD:
    // query = query.neq("servicio", "HD");
  ...
  const handleUpdate = async (e) => {
    ...
    // ✅ LÓGICA CORREGIDA: Eliminada la referencia a HD
    if (formData.servicio === "ESTIVALES" && formData.tipo === "FALSO FLETE" && formData.modalidad === "PAGO") montoFinal = formData.porcentaje;
  ...
  return (
    <div className="min-h-screen px-4 md:px-8 py-8 relative animate-fade-in font-sans">
      {/* Fondo enterprise */}
      ...
      <PageHeader
        ...
        subtitle="Finanzas • Reportería"
        icon={TrendingUp}
      >
        <button
          onClick={() => setViewMode('SEMANA')}
          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
            viewMode === 'SEMANA' ? 'bg-white text-[#0b1f44] shadow-sm' : 'text-white/60 hover:text-white hover:bg-white/10'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
            viewMode === 'MES' ? 'bg-white text-[#0b1f44] shadow-sm' : 'text-white/60 hover:text-white hover:bg-white/10'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
            viewMode === 'HISTORICO' ? 'bg-white text-[#0b1f44] shadow-sm' : 'text-white/60 hover:text-white hover:bg-white/10'
          }`}
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 shadow-sm p-4 relative overflow-hidden group">
      ...
      {/* Centro de descargas */}
      <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 shadow-sm p-5 mb-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="bg-pink-50 p-2 rounded-xl text-[#d63384]">
              <FileSpreadsheet size={24} />
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Centro de Descargas</p>
              <p className="text-sm font-bold text-slate-900 leading-tight">Exportación Excel Personalizada</p>
            </div>
          </div>
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-slate-400 uppercase mb-1">Desde</span>
              <input type="date" value={rangoDescarga.inicio} onChange={(e) => setRangoDescarga({ ...rangoDescarga, inicio: e.target.value })} className="text-xs rounded-xl px-3 py-2 font-bold border border-slate-200 bg-slate-50 text-slate-700 outline-none focus:ring-2 focus:ring-[#d63384]/20" />
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-slate-400 uppercase mb-1">Hasta</span>
              <input type="date" value={rangoDescarga.fin} onChange={(e) => setRangoDescarga({ ...rangoDescarga, fin: e.target.value })} className="text-xs rounded-xl px-3 py-2 font-bold border border-slate-200 bg-slate-50 text-slate-700 outline-none focus:ring-2 focus:ring-[#d63384]/20" />
            </div>
            <button onClick={descargarExcelPorRango} disabled={loadingDescarga} className="h-[38px] rounded-xl bg-[#d63384] hover:bg-pink-600 text-white font-bold px-5 text-xs shadow-md shadow-pink-200 transition active:scale-[0.98] disabled:opacity-60 flex items-center gap-2">
              {loadingDescarga ? "..." : "Descargar Excel"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 shadow-sm p-4 mb-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="flex flex-wrap items-end gap-4">
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Filtrar servicio</label>
              <select value={filtroServicio} onChange={(e) => setFiltroServicio(e.target.value)} className="border border-slate-200 bg-white rounded-xl px-3 py-2 font-bold text-sm text-slate-700 outline-none focus:border-[#1e3c72] shadow-sm cursor-pointer">
                <option value="TODOS">TODOS LOS SERVICIOS</option>
                {Object.keys(localesPorServicio).map((svc) => (<option key={svc} value={svc}>{svc}</option>))}
              </select>
            </div>
            <div>
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Buscar ID</label>
              <div className="relative">
                <input type="text" placeholder="Buscar patente..." value={busquedaPatente} onChange={(e) => setBusquedaPatente(e.target.value)} className="border border-slate-200 bg-white rounded-xl pl-9 pr-3 py-2 font-mono font-bold uppercase text-sm text-slate-700 w-52 outline-none focus:border-[#1e3c72] shadow-sm" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <h6 className="font-black text-[#1e3c72] text-[11px] uppercase tracking-wider mb-4 border-b pb-2">Gasto por servicio ({getPeriodoLabel()})</h6>
          <div className="h-64"><Bar data={datosGraficos.barData} options={{ indexAxis: "y", maintainAspectRatio: false }} /></div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <h6 className="font-black text-[#1e3c72] text-[11px] uppercase tracking-wider mb-4 border-b pb-2">Distribución Conceptos ({getPeriodoLabel()})</h6>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left">
            <thead className="bg-[#0b1f44] text-white uppercase font-black tracking-wider text-[10px]">
              <tr>
                ...
                <th className="px-4 py-4 text-right">Monto Prest.</th>
                <th className="px-4 py-4 text-right">Cobro Cliente</th>
              ...
              {registrosFiltrados.map((reg) => (
                <tr key={reg.id} className="hover:bg-blue-50/50 transition-colors">
                  <td className="px-4 py-3 text-center">
                    <button onClick={() => handleEditClick(reg)} className="text-slate-400 hover:text-[#d63384] transition-colors p-1 hover:bg-pink-50 rounded-lg"><Edit3 size={14} /></button>
                  </td>
                  <td className="px-4 py-3 text-slate-600 font-medium">{reg.fecha_evento}</td>
                  <td className="px-4 py-3 font-bold text-slate-800">{reg.servicio}</td>
                  <td className="px-4 py-3 text-slate-600">{reg.local}</td>
                  ...
                  <td className="px-4 py-3 truncate max-w-[200px] text-slate-400 italic">{reg.comentario}</td>
                </tr>
              ))}
              {registrosFiltrados.length === 0 && (
                <tr><td colSpan={8} className="p-12 text-center text-slate-400 font-bold">Sin datos para este periodo.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* Modal Edición */}
      {showModal && (
        <div className="fixed inset-0 bg-[#0b1f44]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/20">
             <div className="bg-slate-50 px-6 py-4 border-b flex justify-between items-center">
                <h3 className="font-black text-[#1e3c72] uppercase tracking-wider text-sm">Editar Registro</h3>
                <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-900"><X size={20} /></button>
             </div>
             <div className="p-6 max-h-[80vh] overflow-y-auto">
                <form onSubmit={handleUpdate} className="space-y-4">
                     <div className="grid grid-cols-2 gap-4">
                        <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1">FECHA</label><input type="date" name="fecha" value={formData.fecha} onChange={handleModalChange} className="w-full border p-2 rounded-xl font-bold text-slate-700 bg-slate-50 focus:bg-white transition" /></div>
                        <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1">SERVICIO</label><select name="servicio" value={formData.servicio} onChange={handleModalChange} className="w-full border p-2 rounded-xl font-bold text-slate-700 bg-slate-50 focus:bg-white transition"><option value="">Sel...</option>{Object.keys(localesPorServicio).map(s=><option key={s} value={s}>{s}</option>)}</select></div>
                     </div>
                     <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1">LOCAL</label><select name="local" value={formData.local} onChange={handleModalChange} className="w-full border p-2 rounded-xl font-bold text-slate-700 bg-slate-50 focus:bg-white transition"><option value="">Sel...</option>{localesPorServicio[formData.servicio]?.map(l=><option key={l} value={l}>{l}</option>)}</select></div>

                     {formData.modalidad === 'PAGO' && (
                        <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                            <div className="flex gap-4 mb-2">
                                <div className="w-1/2"><label className="text-[10px] font-bold text-blue-900/60 uppercase mb-1">ID / PATENTE</label><input type="text" name="patente" value={formData.patente} onChange={(e) => setFormData({...formData, patente: e.target.value.toUpperCase()})} className="w-full border border-blue-200 p-2 rounded-xl font-mono font-black text-blue-900"/></div>
                                <div className="w-1/2"><label className="text-[10px] font-bold text-blue-900/60 uppercase mb-1">CONCEPTO</label><select name="tipo" value={formData.tipo} onChange={handleModalChange} className="w-full border border-blue-200 p-2 rounded-xl font-bold text-slate-700"><option value="AMBULANCIA">AMBULANCIA</option><option value="FALSO FLETE">FALSO FLETE</option><option value="OTRO">OTRO</option></select></div>
                            </div>
                            <div><label className="text-[10px] font-bold text-blue-900/60 uppercase mb-1">MONTO PRESTADOR</label><input type="text" name="montoPrestador" value={formData.montoPrestador} onChange={handleModalChange} className="w-full border border-blue-200 p-2 rounded-xl font-black text-right text-lg text-blue-900" /></div>
                        </div>
                     )}

                     <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                        <div className="flex gap-4">
                            <div className="w-1/3"><label className="text-[10px] font-bold text-emerald-900/60 uppercase mb-1">COBRO CLIENTE</label><input type="text" name="montoCliente" value={formData.montoCliente} onChange={handleModalChange} className="w-full border border-emerald-200 p-2 rounded-xl font-black text-emerald-700 text-right"/></div>
                            <div className="w-2/3"><label className="text-[10px] font-bold text-emerald-900/60 uppercase mb-1">DETALLE / GLOSA</label><input type="text" name="detalleCliente" value={formData.detalleCliente} onChange={handleModalChange} className="w-full border border-emerald-200 p-2 rounded-xl font-bold text-slate-700"/></div>
                        </div>
                     </div>
                     <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1">COMENTARIO INTERNO</label><textarea name="comentario" value={formData.comentario} onChange={handleModalChange} className="w-full border p-2 rounded-xl text-slate-600 bg-slate-50 focus:bg-white transition" rows="2"></textarea></div>
                     <div className="flex gap-3 pt-2">
                        <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 bg-white border border-slate-200 text-slate-500 hover:text-slate-700 font-bold rounded-xl transition">Cancelar</button>
                        <button type="submit" className="flex-1 py-3 bg-[#1e3c72] hover:bg-[#152c55] text-white font-bold rounded-xl shadow-lg shadow-blue-900/20 transition">Guardar Cambios</button>
                     </div>
                </form>
             </div>
          </div>
